...
export default function WeeklySummaryCard({ weekStart }) {
  ...
  const assignment = currentOfficer?.officer ? {
    ...
    officer_email: currentOfficer.officer?.email,
    role: currentOfficer.officer?.role || 'officer_on_duty',
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="border rounded p-3 bg-white w-full md:w-auto">
          ...
          {assignment ? (
            <UserAvatarWithRole
              user={{
                name: assignment.officer_name,
                email: assignment.officer_email,
                role: assignment.role || 'officer_on_duty',
              }}
              size="sm"
              showEmail={Boolean(assignment.officer_email)}
              className="mt-2"
            />
          ) : (
            <div className="font-semibold text-gray-800 text-sm mt-1">Not assigned</div>
          )}
