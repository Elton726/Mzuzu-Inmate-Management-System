        ...
        <table className="w-full min-w-[950px]">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-gray-700 font-semibold">Security</th>
          ...
          <tbody>
            {list.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-6 text-center text-gray-500">No activities found.</td>
              </tr>
            ) : (
              list.map((a) => (
                <tr key={a.id} className="border-b hover:bg-gray-50 transition">
                  <td className="px-6 py-4 font-semibold text-gray-800">{a.name}</td>
                  <td className="px-6 py-4 text-gray-700">{a?.category?.name || '--'}</td>
                  <td className="px-6 py-4 text-gray-700 capitalize">{a.security_level || '--'}</td>
